 ...
 */
public class MemberPageDto<T> implements Serializable {
	...
	public MemberPageDto(Long totalCount, Integer pageNo, Integer pageSize,
			List<T> items) {
		super();
	...
	public String getPageStr() {
		...
		stringBuffer.append("<span class=\"mod_pagenav_count\">");
		if (pageNo == 1) {
			stringBuffer
					.append("");
		} else {
			stringBuffer.append("<a href=\"#\" title=\"pageSize\" onclick=\"query(" + prePage
					+ "," + pageSize + ")\"><</a>");
		}
		for (int i = pageNo-5; i < pageNo+5; i++) {
			...
			if (i==pageNo) {
				stringBuffer
				.append("<a href=\"#\" title=\"pageSize\" onclick=\"query("
							+ i + "," + pageSize + ")\">"+i+"</a>");
			}else {
				stringBuffer
				.append("<a href=\"#\" title=\"pageSize\" onclick=\"query("
