		...
		<table class="table table-hover">
			...
			<tbody>
				<c:forEach items="${list.items}" var="user">
					<c:if test="${login_user.id eq user.id }">
						<tr>
							<td>${user.id}</td>
							<td>
							...
							<td class="disable">
								<a class="disable" href="<%=path %>/admin/user/edit?id=${user.id}">编辑</a> |
								<c:if test="${user.status eq 0 }">
									<a class="disable" href="javascript:void(0);" onclick="changeStatus(${user.id});" style="color:#3c8dbc">解冻</a> |
								</c:if>
								<c:if test="${user.status eq 1 }">
									<a class="disable" href="javascript:void(0);" onclick="changeStatus(${user.id});" style="color:crimson">冻结</a> |
								</c:if>
								<a class="disable" href="javascript:void(0);" onclick="del(${user.id});" style="color:crimson">永久删除</a> 
							</td>
						</tr>
					</c:if>
